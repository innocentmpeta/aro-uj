...
import { ArrowRight, ShieldCheck, Users, Link2, Ban, Flag, PenLine, MapPin, RotateCcw, Send } from 'lucide-react'
...
export default function StudentPracticumPage() {
  ...
  const PAST_HIGHLIGHTS = projects.slice(0, 3)
  return (
    <div className="bg-white">
      ...
      <PageHero
        ...
        eyebrow="For UJ students"
        title="Student Practicum"
        lead="Apply your expertise in the field. Work directly with ARO members on real challenges — and learn from the community as much as from the classroom."
        variant="dark"
      />

      {/* ── WHO CAN APPLY ─────────────────────────────────────────────── */}
      {sections.practicum_who !== false && (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            <div>
              ...
              <p className="text-body text-muted mb-8">
                Reclaimers face challenges that require every kind of expertise.
                Engineers design safer working spaces. Law students navigate UIF
                and documentation barriers. FADA students communicate reclaimer
                stories with dignity. Humanities researchers analyse policy.
                Health Sciences students assess occupational risks. Economists
                study market dynamics.
              </p>
              <p className="text-body text-muted">
                There is no faculty that does not have something to contribute.
                If you can see a connection between your discipline and the
                challenges reclaimers face, that connection is where your
                practicum begins.
              </p>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {['FADA', 'Law', 'Engineering (FEBE)', 'Humanities', 'Science', 'Health Sciences', 'CBE', 'Education'].map(f => (
                <div key={f}
                  className="bg-white rounded-xl border border-border px-4 py-3 font-body text-small text-ink">
                  {f}
                </div>
              ))}
            </div>
        ...
        <div className="container">
          ...
          <div className="mt-10 space-y-px">
            {HOW_IT_WORKS.map(({ step, heading, body }) => (
              <div key={step}
                className="flex items-start gap-8 p-7 bg-white border border-border rounded-2xl mb-3">
                <div className="font-body text-xs font-semibold text-forest/40 tracking-widest shrink-0 pt-0.5 w-6">
                  {step}
                </div>
                <div>
      ...
      {/* ── PAST HIGHLIGHTS ───────────────────────────────────────────── */}
      {sections.practicum_past !== false && (
      <section className="section bg-surface">
        <div className="container">
          <p className="eyebrow">Past practicum highlights</p>
          <h2 className="section-heading">What students have built</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-10">
            {PAST_HIGHLIGHTS.map((project: any) => (
              <div key={project.id ?? project.title} className="card bg-white">
                ...
                <div className="p-6">
                  ...
                  <p className="font-body text-xs text-muted mb-3">{project.outcome}</p>
        ...
        <div className="container max-w-3xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-8">
            {([
              ...
              <div key={heading} className="rounded-2xl border border-border p-6">
